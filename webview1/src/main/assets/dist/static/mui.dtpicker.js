 ...
 * Houfeng@DCloud.io
 * 
 * edit by archer 2016年03月15日 不出现未来的日期
 * 		配置项 option.endTimeIsNow = true 的时候开启
 */
	...
	var DtPicker = $.DtPicker = $.Class.extend({
		init: function(options) {
			...
			ui.y.addEventListener('change', function() {
				// edit by archer @ 以便隐藏掉未来的day
				if (self._getEndTimeIsNow()) {
					self._createMonth();
				}
				self._createDay();
			}, false);
			...
			if (self._getEndTimeIsNow()) {
				ui.d.addEventListener('change', function() {
					self._createHours();
				}, false);
				ui.h.addEventListener('change', function() {
					self._createMinutes();
				}, false);
			}
		...
		getDayNum: function(year, month) {
			...
			if (self._getEndTimeIsNow()) {
				try {
					...
					if (year && year == now.getFullYear() && month && month == nowM) {
						maxD = now.getDate();
					}
				} catch (e) {
					// do nothing
			...
			if (self._inArray([1, 3, 5, 7, 8, 10, 12], month)) {
				return Math.min(maxD, 31);
			} else if (self._inArray([4, 6, 9, 11], month)) {
				return Math.min(maxD, 30);
			} else if (self.isLeapYear(year)) {
				return Math.min(maxD, 29);
			} else {
				return Math.min(maxD, 28);
			}
		},
		_fill: function(num) {
			num = num.toString();
		...
		// add by archer @ 获取是否隐藏掉未来的日期和时间
		_getEndTimeIsNow: function() {
			var self = this;
			var options = self.options;
			return options.endTimeIsNow ? true : false;
		},
		_createMonth: function(current) {
			...
			if (options.customData.m) {
				...
				// edit by archer @ 选择年份为当前年的时候，获取当前的月份，以便隐藏掉未来的月份
				var maxM = 12;
				if (self._getEndTimeIsNow()) {
					try {
						var selectedY = ui.y.picker.getSelectedValue();
						var now = new Date();
						if (selectedY && selectedY == now.getFullYear()) {
							maxM = now.getMonth() + 1;
						}
					} catch (e) {
						// do nothing
		...
		_createHours: function(current) {
			...
			if (options.customData.h) {
				...
				if (self._getEndTimeIsNow()) {
					try {
						var selectedY = ui.y.picker.getSelectedValue();
						var selectedM = ui.m.picker.getSelectedValue();
						var selectedD = ui.d.picker.getSelectedValue();
						var now = new Date();
						var nowY = now.getFullYear();
						var nowM = now.getMonth() + 1;
						var nowD = now.getDate();
						if (selectedY && selectedY == nowY &&
							selectedM && selectedM == nowM &&
							selectedD && selectedD == nowD) {
							maxH = now.getHours();
						}
					} catch (e) {
						// do nothing
			...
			if (options.customData.i) {
				...
				if (self._getEndTimeIsNow()) {
					try {
						...
						var selectedM = ui.m.picker.getSelectedValue();
						var selectedD = ui.d.picker.getSelectedValue();
						var selectedH = ui.h.picker.getSelectedValue();
						var now = new Date();
						var nowY = now.getFullYear();
						var nowM = now.getMonth() + 1;
						var nowD = now.getDate();
